...
const ClearFormButton = ({ onClear, disabled = false }: Props) => (
  <AlertDialog>
    ...
    <AlertDialogContent>
      <AlertDialogHeader>
        <AlertDialogTitle>Anketi temizle?</AlertDialogTitle>
        <AlertDialogDescription>
          Anketi temizlemek istediğinizden emin misiniz? Bu eylem geri alınamaz
          ve mevcut formdaki tüm ilerlemeyi kalıcı olarak kaldıracaktır.
        </AlertDialogDescription>
      </AlertDialogHeader>
...
export default function GeneratedForm() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => toast.success('Anket başarıyla gönderildi'),
    onError: () => toast.error('Anketi göndermede hata oluştu'),
  });

  useTitle(data?.name || 'AnketKOU');
            ...
            <ul className="space-y-5">
              ...
              {data.isActive ? null : (
                <li className="text-sm font-medium text-rose-700">
                  * Not: Bu anketin durumu kapalıdır.
                </li>
              )}
