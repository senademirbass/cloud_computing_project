...
export default function CreateForm({ formType = 'add', form }: Props) {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(
        `Anket başarıyla ${formType === 'add' ? 'oluşturuldu' : 'güncellendi'}`,
      );
    },
    onError: () =>
      toast.error(
        `Anket ${formType === 'add' ? 'oluşturmada' : 'güncellemede'} hata`,
      ),
  });
        ...
        >
          <section className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-3 whitespace-nowrap">
              <label className="font-medium">Anket Adı:</label>
              <Input
                required
          ...
          <section className="mt-5 flex items-center gap-5 self-end">
            ...
            {formElements.length !== 0 ? (
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>Sıfırla</AlertDialogTitle>
                    <AlertDialogDescription>
                      Anketi temizlemek istediğinizden emin misiniz? Bu,
                      yaptığınız tüm değişiklikleri geri alınamaz bir şekilde
                      siler.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
            ...
            >
              ...
              <span>{form ? 'Anketi Güncelle' : 'Anketi Kaydet'}</span>
            </Button>
          </section>
        </form>
      </div>
